...
function renderMenu(kategori) {
    ...
    if(filtered.length === 0) {
        return daftarMenuEl.innerHTML = `<p style="grid-column:1/-1; text-align:center; color:#888;">Tidak ada menu kategori ini.</p>`;
    }
...
function updateCartUI() {
    ...
    totalPriceEl.innerHTML = `<div style="display:flex; justify-content:space-between; font-size:13px; color:#ccc;"><span>Subtotal</span><span>${subtotal.toLocaleString('id-ID')}</span></div>`;
...
btnCheckout.addEventListener('click', () => {
    ...
    payTotalDisplay.innerText = "Rp " + formatNum(grand); payInput.value = ""; payChange.innerText = "Rp 0";
